...
- 从技能 `2.2.72` 开始，`cloudcc create trigger <TriggerName>` 生成一级 `backend/triggers/<TriggerName>` 目录和真正的 `CCTrigger` 外层类，SOURCE 标记固定放在构造函数内；`objectApi/TriggerName` 目录形式继续兼容但只用于组织。触发器对象归属以 `config.json.targetObjectId` 为准，触发时机以 `triggerTime` 为准；未绑定骨架在 publish 网络请求前失败。定时类不属于业务对象，由 `scheduleJob.prgid` 绑定运行。
