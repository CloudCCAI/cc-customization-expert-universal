...
触发器生成或修改后可运行 `cloudcc format trigger <object/TriggerName> <projectPath> --write` 显式修复，并用 `--check` 只读复核。CLI 使用内置 `google-java-format 1.29.0 --aosp` 强制 4 空格缩进和普通语句一行一条；纯 SOURCE 语句片段会在临时包装成合法 Java 编译单元后格式化并安全提取。从 CLI `2.2.69` 开始，publish 会自动格式化并写回本地触发器源码，成功后继续远程 validate/save；只有格式器失败才会以 `blocked_local_format` 在远程请求前终止，发布结果通过 `localFormat` 报告是否发生写回。
...
旧版触发器包可能使用 `devid`（或 `devId`）保存线上记录 ID。发布时 CLI 按 `id`、`devid`、`devId` 的顺序解析：有效 `id` 始终优先，仅当 `id` 缺失或为空时才兼容旧字段；三者都不存在时才按新增触发器处理。跨环境复制旧包时仍需先确认该 ID 属于目标环境，不能把来源环境 ID 当作目标环境 ID。
