...
MetadataService object spec 可传 `labels`、`translations`、`multiLang` 或 `multiLanguages` 数组；每项使用 `country|locale|language|lang`、`label|labelName|name|value` 和 `isDefault|isdefault`。显式数组存在时，服务逐条保存全部语言及默认标志，不自动缩减为 zh/en。
...
- 不传 `profiles`、`profilePermissions` 或 `profileFieldArr` 时，系统管理员简档
  `aaa000001` 获得只读、新建、编辑、删除、查看全部、修改全部六项权限；其它租户简档
  获得只读、新建、编辑、删除权限，不获得查看全部和修改全部。
- 显式传入简档时，按传入的简档和六位权限值分配；如果未包含系统管理员，
  MetadataService 自动补入 `aaa000001`，权限为 `1,1,1,1,1,1`。
- 对象简档权限没有“简档可见/简档默认”之分；这两个状态只用于记录类型的简档绑定。
